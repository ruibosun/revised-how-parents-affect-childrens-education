---
title: "Exploring the Relation between Family Background and Children's Education Level in the United States (2021)"
author: 
  - Ruibo Sun
...
$$
Prestige_{ij}=\mu+\alpha_i+\beta_{j}+\epsilon_{ij}
$$ {#eq-modpr}
...
The number of oberservation before data cleaning is 4032. This number decrease to 3794 because some rows with missing values. are dropped from the data.

In addtion, the variable of **degree** is grouped into two levels. For respondents who have obtained at least a degree from an associate/junior college, their levels are denoted as "1", indicating they have complete a post-secondary education. For respondents who have attained only a high school diploma or possess a lower level of education, their levels are designated as "0".
...
tb1<-tb1%>%arrange(factor(tb1$degree,c("graduate", "bachelor's", "associate/junior college", "high school", "less than high school")))
...
tb0 <- cleaned_data %>% summarise(Min = min(cleaned_data$educ,na.rm=T),
                              ...
                              Mean = round(mean(cleaned_data$educ,na.rm=T),digits=2))
...
ggplot(cleaned_data, aes(x = mapres10)) +
  ...
  theme_bw()
```
In @fig-parentpre, it shows the distribution of respondent’s father’s and mother’s prestige scores. The distribution of father’s prestige score and the mother’s prestige score have quite different distributions. Specifically, the distribution of mother's prestige score is bimodal and the distribution of father's prestige score is unimodal.
...
ggplot(cleaned_data, aes(x = maeduc)) +
  geom_histogram(aes(y = ..density.., fill = "Mother's Education"),
                 alpha = 1, binwidth = 0.5, color ="black") +geom_histogram(aes(x = paeduc, y = ..density.., fill = "Father's Education"),alpha = 0.8, binwidth = 0.5, color = "black") +
...
ggplot(cleaned_data, aes(x = educ, fill = factor(born))) +
  geom_histogram(aes(y = ..density..), position = "dodge", binwidth = 1.5, alpha = 0.5, color = "black")+
   labs(x = "Education (in Years)",
       y = "Density",
       fill = "Birthplace of the Respondent (if in US)")+
  theme_bw()

# ggplot(cleaned_data, aes(x = educ)) +
...
# theme_bw()
```
Based on [@overman], the birthplace may have an impact on children’s education. In @fig-birth, it is used to test the relationship between birthplace and children’s education. Note that the question of birthplace only asks if the respondent was born in the US or not. In general, most of the respondents have received at least 10 years of education despite their birthplace. However, the bar for respondents who were born outside of the US is much higher than the other groups. This is because 90% of the respondents were born in the US, and only 10.0% answered “No”. This is because respondents who did not answer this question or answered NA are removed during the data cleaning. Given the huge differences in the sample sizes between groups, it is hard to draw a general and representative conclusion of whether birthplace may affect the education level.
...
$$
\log(\frac{\pi}{1-\pi})=\hat{\beta_0}+\hat{\beta_1}X_{mapres10}+\hat{\beta_2}X_{papres10}+\hat{\beta_3}X_{paeduc}+\hat{\beta_4}X_{maeduc}
$$ {#eq-model}
...
# Fit the model
model <- glm(high_school ~ mapres10 + papres10 + paeduc + maeduc + incom16 + reg16, family="binomial", data = cleaned_data)
